...
public class Chriki_Ismael_amigo_invisible {
	...
	public static String[][] asignarParticipantes(Scanner sc) {
		...
		//Verificiar si el usuario a introducido dos nombres iguales
		if(!nombreDuplicado()) {
			System.out.println("Participantes guardados correctamente\n");
			return participantes;
		}
			System.out.println("Has introducido a dos o mas participantes con el mismo nombre.\nPor favor, introduzca otros nombres");
	...
	public static void mostrarSorteo() {
		...
		if(participantes==null || participantes[1][0]==null) {
			System.out.println("No se puede mostrar las asignaciones sin haber realizado el sorteo\n");
			return;
		}
	...
	public static void sorteo() {
		int[] orden=new int[numeroParticipantes];
				...
				for(int i=0;i<numeroParticipantes;i++) {
					if(i==orden[i]) {
						int aux = (i+1)%numeroParticipantes;
						int temp=orden[i];
						orden[i]=orden[aux];
						orden[aux]=temp;
					}
	...
	public static void repetirSorteo() {
		if(participantes==null || participantes[1][0]==null) {
			System.out.println("No se puede realizar nueva mezcla sin haber realizado el sorteo primero\n");
			return;
		}
		for(int i=0;i<numeroParticipantes;i++) {
	...
	//Verificar si los nombres introducidos son iguales
	public static boolean nombreDuplicado() {
		for(int i=0;i<numeroParticipantes;i++) {
			for(int j=0;j<numeroParticipantes;j++) {
				if(i!=j && participantes[0][i].equals(participantes[0][j])) {
					return true;
				}
			}
		}
